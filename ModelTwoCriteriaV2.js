...
const createFilteredVectors = (dictionary, excludedWords, trainingData) => {
  const words = Object.keys(dictionary);
  ...
  // Добавить векторы из trainingData
  trainingData.forEach((text) => {
    vectors.push(ConvertToVector(text));
  });
...
let filteredVectors = createFilteredVectors(WordDictionary, excludedWords, trainingData);
